...
def getMagnitude(Gx, Gy, height, width):
    ...
    for row in range(height):
        for column in range(width):
            gx = np.clip(Gx[row][column], -1e10, 1e10)
            gy = np.clip(Gy[row][column], -1e10, 1e10)
            gradientData[row][column] = ((gx ** 2 + gy ** 2) ** 0.5) / 1.4142
    return gradientData
...
def getAngle(Gx, Gy, height, width):
    ...
    for i in range(height):
        for j in range(width):
            gx = np.nan_to_num(Gx[i][j], nan=0.0, posinf=0.0, neginf=0.0)
            gy = np.nan_to_num(Gy[i][j], nan=0.0, posinf=0.0, neginf=0.0)
            angle = math.degrees(math.atan(gy / (gx + epsilon)))
            if angle < 0:
                angle += 360
...
if __name__ == "__main__":
    ...
    image = cv2.imread('images/birdyy.jpg', 0)
