...
class FindEllipseRHT(object):
    ...
    def __init__(self, phase_img, mask):
        ...
        # edge image (expected to be pre-processed)
        self.edge = None
    ...
    def prepare_edge_image(self, img):
        """
        Prepare the edge image for processing.
        Note: This function expects an already edge-detected image.
        """
        return img
    ...
    def run(self, debug_mode=False, plot_mode=False):
        ...
        # prepare edge image
        self.edge = self.prepare_edge_image(self.origin)
        ...
        for count, i in enumerate(range(self.max_iter)):
            ...
            pad_black = cv2.ellipse(pad_black, (center_plot[0] + pad_width, center_plot[1] + pad_width), axis_plot,
                                    angle * 180 / np.pi, 0, 360, color=255, thickness=1)
    ...
    def plot_best(self, p, q, a, b, angle):
        ...
        plt.figure()
        plt.imshow(crop)
        plt.title("Best Ellipse Fit")
        plt.show()

    def inner_outer_phase(self, p, q, a, b, angle):
        ellipse_label = np.zeros(self.origin.shape, dtype=np.uint8)
        if a > b:
            ellipse_label = cv2.ellipse(ellipse_label, (p, q), (a, b), angle * 180 / np.pi, 0, 360, color=255,
                                        thickness=-1)
        else:
            ellipse_label = cv2.ellipse(ellipse_label, (p, q), (b, a), angle * 180 / np.pi, 0, 360, color=255,
                                        thickness=-1)
    ...
    def find_center(self, pt, plot_mode=False):
        ...
        for i in range(len(pt)):
            ...
            # Boundary checks
            xstart = max(0, xstart)
            xend = min(self.edge.shape[1], xend)
            ystart = max(0, ystart)
            yend = min(self.edge.shape[0], yend)

            crop = self.edge[ystart: yend, xstart: xend].T
            proximal_point = np.array(np.where(crop == 255)).T

            if len(proximal_point) == 0:
                raise np.linalg.LinAlgError("No edge points found in the proximity")
        ...
        if float('inf') in slope_arr:
            idx = slope_arr.index(float('inf'))
            x = intercept_arr[idx]
            ...
            y = slope_arr[other_idx] * x + intercept_arr[other_idx]
            center = np.array([x, y])
        else:
            # find center
    ...
    def find_semi_axis(self, pt, center):
        ...
        if self.assert_valid_ellipse(A, B, C):
            ...
            AXIS_MAT = np.array([[np.sin(angle) ** 2, np.cos(angle) ** 2],
